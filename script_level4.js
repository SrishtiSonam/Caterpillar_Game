...
var excludedValuesFoodX = [14,15,16];
...
const changeFoodPosition = () => {
    ...
    }while( (foodY==1) || (foodY==30) || ( (foodY==15) && (!(excludedValuesFoodX.includes(foodX))) ) )
}
...
const boarder = () => {
    ...
    for (i=1;i<31;i++) {
        for(j=1;j<31;j++){
            if ( (j==1) || (j==30) || ( (j==15) && (!(excludedValuesFoodX.includes(i))) ) ){
...
const initGame = () => {
    ...
    if( (caterpillarY==1) || (caterpillarY==30) || ( (caterpillarY==15) && (!(excludedValuesFoodX.includes(caterpillarX))) ) ) {   
        gameOver = true;
    }
